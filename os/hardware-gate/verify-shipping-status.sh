...
check "capture driver fail-closes on missing VM stages" "rg -q 'GOS_SERIALLOG' os/hardware-gate/capture-harness/run-capture.sh os/hardware-gate/capture-harness/drive-capture.py && rg -q 'wait_serial_contains(\"ISO boot menu\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'observe_serial_contains(\"ISO boot handoff\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'continuing to framebuffer stages' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'key(\"ret\")' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'require_frame(\"Anaconda summary\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'require_frame(\"first boot desktop\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'framebuffer timeout after' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'did not appear' os/hardware-gate/capture-harness/drive-capture.py"
