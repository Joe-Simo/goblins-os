...
- [ ] **First-boot page + live copy job (deferred, CI/qemu):** the installer "Bring your stuff over" branch, source selection UI over `/v1/migration/sources`, read-only `udisksctl` mount for unmounted chosen sources, running the planned `rsync` copy with a Copied/Skipped ledger, and the allowlisted dconf→gsettings preference import.
