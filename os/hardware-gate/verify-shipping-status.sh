...
check "capture harness boots verification ISO once then installed disk" "rg -q -- '-boot order=c,once=d' os/hardware-gate/capture-harness/run-capture.sh && rg -q 'verification ISO once and then prefers the installed VM disk' os/hardware-gate/runbook.md"
