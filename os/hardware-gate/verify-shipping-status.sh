...
check "capture driver fail-closes on serial VM stages and diagnostic frames" "rg -q 'GOS_SERIALLOG' os/hardware-gate/capture-harness/run-capture.sh os/hardware-gate/capture-harness/drive-capture.py && rg -q 'wait_serial_contains(\"ISO boot menu\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'observe_serial_contains(\"ISO boot handoff\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'continuing to framebuffer stages' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'key(\"ret\")' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'wait_stage(\"Anaconda storage confirmation\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'Anaconda destination disk selected' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'Anaconda summary after destination' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'Anaconda begin submitted' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'click(0.937, 0.895)' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'wait_serial_contains(\"kickstart install post\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'GOBLINS_VERIFY_INSTALL_DONE' os/hardware-gate/capture-harness/drive-capture.py os/iso/verify-install.ks && rg -q 'wait_stage(\"first boot desktop\"' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'diagnostic framebuffer samples' os/hardware-gate/capture-harness/drive-capture.py && rg -q '_debug-' os/hardware-gate/capture-harness/drive-capture.py && ! rg -q 'require_frame\\(' os/hardware-gate/capture-harness/drive-capture.py && ! rg -q 'wait_frame\\(' os/hardware-gate/capture-harness/drive-capture.py"
check "capture harness uses QMP absolute pointer range" "rg -q 'ABS_MAX = 0x7fffffff' os/hardware-gate/capture-harness/drive-capture.py os/hardware-gate/capture-harness/qmp-capture.py && rg -q 'abs_axis(value)' os/hardware-gate/capture-harness/drive-capture.py os/hardware-gate/capture-harness/qmp-capture.py && ! rg -q '32767' os/hardware-gate/capture-harness/drive-capture.py os/hardware-gate/capture-harness/qmp-capture.py"
