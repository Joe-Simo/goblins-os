...
check "capture driver dismisses first boot through private path with debug frame" "rg -q 'first boot setup: selecting welcome window and clicking private offline path' os/hardware-gate/capture-harness/drive-capture.py && rg -q 'post first boot dismiss' os/hardware-gate/capture-harness/drive-capture.py"
