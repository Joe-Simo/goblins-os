...
check "capture fixture core uses served local model" "rg -Fq 'CAPTURE_LOCAL_MODEL=\"\${GOBLINS_OS_LOCAL_MODEL:-llama3.2:1b}\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_MODEL=\"\$CAPTURE_LOCAL_MODEL\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'ollama pull llama3.2:1b' .github/workflows/hardware-gate-capture.yml"
