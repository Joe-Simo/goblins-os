...
check "hardware gate session automation uses verification-only service not Alt+F2 injection" "rg -q 'goblins-hwgate-session-orchestrator.service' os/iso/verify-config.toml && rg -q '99-goblins-hwgate-session-orchestrator.conf' os/iso/verify-config.toml && rg -q 'download_with_wait firstboot-unlock.sh /tmp/gos-firstboot 240' os/iso/verify-config.toml && rg -q 'download_with_wait orchestrator.sh /tmp/gos-orchestrator 600' os/iso/verify-config.toml && rg -q 'GOS_ORCHESTRATOR_DEST' os/hardware-gate/capture-harness/run-capture.sh && rg -q 'publish_orchestrator()' os/hardware-gate/capture-harness/drive-capture.py && rg -q '\"GET /orchestrator.sh HTTP/1.1\" 200' os/hardware-gate/capture-harness/drive-capture.py && ! rg -q 'key[(]\"alt[+]f2\"|run_alt_f2' os/hardware-gate/capture-harness/drive-capture.py"
