...
check "capture harness restarts aarch64 disk-only after install marker" "rg -q 'aarch64:install' os/hardware-gate/capture-harness/run-capture.sh && rg -q -- '-boot d -no-reboot' os/hardware-gate/capture-harness/run-capture.sh && rg -q 'aarch64:firstboot' os/hardware-gate/capture-harness/run-capture.sh && rg -q 'GOS_EXIT_AFTER_INSTALL_MARKER' os/hardware-gate/capture-harness/run-capture.sh os/hardware-gate/capture-harness/drive-capture.py && rg -q 'GOS_SKIP_INSTALL_PHASE' os/hardware-gate/capture-harness/run-capture.sh os/hardware-gate/capture-harness/drive-capture.py && rg -q 'two-phase capture because QEMU aarch64' os/hardware-gate/runbook.md"
...
check "capture fixture core uses served local model" "rg -Fq 'CAPTURE_LOCAL_MODEL=\"\${GOBLINS_OS_LOCAL_MODEL:-llama3.2:1b}\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'CAPTURE_MODEL_RUNTIME_URL=http://127.0.0.1:41134' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'CAPTURE_MODEL_RELAY_URL=http://127.0.0.1:41135/v1/resident' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'CAPTURE_MODEL_KEEP_ALIVE=\"\${GOBLINS_OS_LOCAL_MODEL_KEEP_ALIVE:-30m}\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'start_capture_model_loopback' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'start_capture_model_contract_relay' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'LISTEN = (\"127.0.0.1\", 41134)' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'LISTEN = (\"127.0.0.1\", 41135)' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'TARGET = (\"10.0.2.2\", 11434)' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq '\\\"keep_alive\\\":\$CAPTURE_MODEL_KEEP_ALIVE_JSON' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_MODEL=\"\$CAPTURE_LOCAL_MODEL\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_MODEL_RELAY=\"\$CAPTURE_MODEL_RELAY_URL\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_MODEL_KEEP_ALIVE=\"\$CAPTURE_MODEL_KEEP_ALIVE\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_RUNTIME_URL=\"\$CAPTURE_MODEL_RUNTIME_URL\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq '/tmp/model-direct.json' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq '/tmp/model-contract-direct.json' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'contract_log_tail=' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'core_log_tail=' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'ollama pull llama3.2:1b' .github/workflows/hardware-gate-capture.yml"
...
for arch in "${ARCHES[@]}"; do
  ...
  if [ -d "$SCREENSHOT_ROOT/$arch" ]; then
    ...
    if [ -n "$LATEST_ARCH_RUN" ]; then
      ...
      print_verification_and_public_release_iso_detail "$LATEST_ARCH_RUN" "$arch"
    else
