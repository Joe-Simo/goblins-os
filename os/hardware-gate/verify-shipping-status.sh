...
check "capture fixture core uses served local model" "rg -Fq 'CAPTURE_LOCAL_MODEL=\"\${GOBLINS_OS_LOCAL_MODEL:-llama3.2:1b}\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'CAPTURE_MODEL_RUNTIME_URL=http://127.0.0.1:41134' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'CAPTURE_MODEL_KEEP_ALIVE=\"\${GOBLINS_OS_LOCAL_MODEL_KEEP_ALIVE:-30m}\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'start_capture_model_loopback' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'LISTEN = (\"127.0.0.1\", 41134)' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'TARGET = (\"10.0.2.2\", 11434)' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq '\"keep_alive\":\$CAPTURE_MODEL_KEEP_ALIVE_JSON' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_MODEL=\"\$CAPTURE_LOCAL_MODEL\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_MODEL_KEEP_ALIVE=\"\$CAPTURE_MODEL_KEEP_ALIVE\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'GOBLINS_OS_LOCAL_RUNTIME_URL=\"\$CAPTURE_MODEL_RUNTIME_URL\"' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq '/tmp/model-direct.json' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'core_log_tail=' os/hardware-gate/capture-harness/in-session-orchestrator.sh && rg -Fq 'ollama pull llama3.2:1b' .github/workflows/hardware-gate-capture.yml"
